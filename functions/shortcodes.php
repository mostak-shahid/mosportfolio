...
function shortcodes_page(){
	...
	<div class="wrap">
		...
		<ol>
			...
			<li>[content wrapper_element='div' wrapper_atts='' limit='' text='' link=''] <span class="sdetagils">displays content of post according to settings</span></li>
...
function site_identity_func( $atts = array(), $content = null ) {
	...
	$logo_url = ($mosportfolio_options['logo']['url']) ? $mosportfolio_options['logo']['url'] : get_template_directory_uri(). '/images/logo.png';
...
function mos_phone_number( $atts, $content = null ) {
    ...
    $default = shortcode_atts( array(
        'number' => $mosportfolio_options['contact-phone']['0'],
    ), $atts );
    ...
    if ($default['number'])
    	...
    	$html .= preg_replace('/[^0-9]/', '', $mosportfolio_options['contact-phone']['0']);
    $html .= '" >';
    if (!$content)
    	$html .= $mosportfolio_options['contact-phone']['0'];
	else
	    $html .= $content;    	
    $html .= '</a>';
    $html .= '</span>';
...
function social_menu_fnc( $atts = array(), $content = '' ) {
	...
	$contact_address = $mosportfolio_options['contact-address'];
	$atts = shortcode_atts( array(
		'google_review' => '',
	), $atts, 'social_menu' );
	...
	foreach ($contact_social as $social) :	
		if ($social['link_url'] AND $social['basic_icon']) :
			...
			$html .= '<li class="social-list '.strtolower(preg_replace('/\s+/', '_', $social['title'])).'"><a href="'. $social['link_url'] .'"';
			if ($social['target'])
				$html .= ' target="_blank"';
	...
	if ($atts['google_review']) {
		foreach ($contact_address as $address) {
			if ($address['review_link'] AND $address['review_link_img']) {
				$html .= '<li class="social-review"><a href="'. $address['review_link'] .'" target="_blank"><span class="social-img"><img src="'.do_shortcode($address['review_link_img']).'" alt="'.$alt_tag['social'] . 'Google Review"></span>';
...
function content_func( $atts = array(), $content = '' ) {
	...
	if ($atts['limit']) $html .= wp_trim_words(get_the_content(), $atts['limit'], '');
...
function phone_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= '<span class="phone-number">';
	    $html .= '<a class="clickToShow" href="tel:';
	    $html .= preg_replace('/[^0-9]/', '', $mosportfolio_options['contact-phone'][$i]);
	    $html .= '" >';
	    $html .= $mosportfolio_options['contact-phone'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($mosportfolio_options['contact-phone'] as $phone) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="phone-number">';
			    $html .= '<a class="clickToShow" href="tel:';
...
function fax_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
		...
		foreach ($mosportfolio_options['contact-fax'] as $fax) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="fax-number">';
			    $html .= '<a class="clickToShow" href="tel:';
			    $html .= preg_replace('/[^0-9]/', '', $fax);
...
function hours_func( $atts = array(), $content = '' ) {
	...
	foreach ($mosportfolio_options['contact-hour'] as $hour) :
    $html .= '<li>' . $hour . '</li>';
	endforeach;
    $html .= '</ul>'; 
    $html .= '</div>'; 
