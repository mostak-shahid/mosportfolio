		...
		?>
			<div class="<?php if($page_layout != 'ns') echo 'row'; else echo 'no-row' ?>">
				...
				<?php if ( have_posts() ) :?>					
					...
					<?php 
					$tab_group_details = get_post_meta( get_the_ID(), '_mosportfolio_tab_group_details_group', true ); 
					if ($tab_group_details) :  ?>
						<?php foreach ($tab_group_details as $tab_single_details) : ?>
							<?php $udi = rand(100, 999) ?>
					<div class="tab-container">
						<h2 class="tab-title"><?php echo do_shortcode( $tab_single_details["_mosportfolio_tab_group_title_text"] );?></h2>
						<?php if ($tab_single_details["_mosportfolio_tab_group_title_images_id"]) : ?>
						<img class="img-responsive img-centered img-tab-title" src="<?php echo wp_get_attachment_url($tab_single_details["_mosportfolio_tab_group_title_images_id"]) ?>" alt="<?php echo $alt_tag["inner"] . $tab_single_details["_mosportfolio_tab_group_title_text"] ?>">
						<?php endif; ?>
						<?php if (sizeof($tab_single_details['_mosportfolio_tab_group_tab_details']) > 1)) : ?>
						<ul class="nav nav-tabs">
							<?php $n = 0;?>
							<?php foreach ($tab_single_details['_mosportfolio_tab_group_tab_details'] as $value) : ?>
