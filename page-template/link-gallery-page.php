		...
		*/
		...
		<?php if($page_layout != 'ns') : ?>
			...
			<?php endif; ?>
				<?php if ( have_posts() ) :?>	
					<?php 
					$gallery_location = get_post_meta( get_the_ID(), '_mosportfolio_link_gallery_location', true );
					$gallery_images = get_post_meta( get_the_ID(), '_mosportfolio_link_gallery_details_group', true );
					$layout = ( get_post_meta( get_the_ID(), '_mosportfolio_link_gallery_layout', true ) ) ? get_post_meta( get_the_ID(), '_mosportfolio_link_gallery_layout', true ) : '6';
					$gallery_gap = get_post_meta( get_the_ID(), '_mosportfolio_link_gallery_gap', true );
					
					$image_width =  get_post_meta( get_the_ID(), '_mosportfolio_link_image_width', true );
					$image_height =  get_post_meta( get_the_ID(), '_mosportfolio_link_image_height', true );
					?>
					...
					if($gallery_images) : ?>
						...
						<?php foreach ( $gallery_images as $gallery_image ) : ?>
							<div class="col-md-<?php echo $layout; if ($gallery_gap) echo ' mb30'; else echo ' no-padding'; ?>">
								<?php //echo $gallery_image['_mosportfolio_link_gallery_details_text'] . ', ' . $gallery_image['_mosportfolio_link_gallery_details_url'] . ', ' . $gallery_image['_mosportfolio_link_gallery_details_image_id'] . ', ' . $gallery_image['_mosportfolio_link_gallery_details_image'] . '<br />'?>
								<?php
								$attachment_id = $gallery_image['_mosportfolio_link_gallery_details_image_id'];
								...
								<div class="img-container">					 
									...
									<span class="text"><?php echo $gallery_image['_mosportfolio_link_gallery_details_text'] ?></span>
									<div class="link-container">
										<a href="<?php echo do_shortcode( $gallery_image['_mosportfolio_link_gallery_details_url'] ) ?>">View Details</a>
									</div>
