...
<div id="comments">
	...
	<?php if (have_comments()) { ?>
		...
		<?php if (get_comment_pages_count() > 1 && get_option('page_comments')) { // are there comments to navigate through  ?> 
			<h3 class="screen-reader-text sr-only"><?php _e('Comment navigation', 'mosportfoliotemplate'); ?></h3>
			<ul id="comment-nav-above" class="comment-navigation pager" role="navigation">
				<li class="nav-previous previous"><?php previous_comments_link(__('&larr; Older Comments', 'mosportfoliotemplate')); ?></li>
				<li class="nav-next next"><?php next_comments_link(__('Newer Comments &rarr;', 'mosportfoliotemplate')); ?></li>
	...
	if (!comments_open() && '0' != get_comments_number() && post_type_supports(get_post_type(), 'comments')) { ?> 
		<p class="no-comments"><?php _e('Comments are closed.', 'mosportfoliotemplate'); ?></p>
	<?php 
	} //endif; 
	?> 
		...
		array(
			'fields' => array(
				'author' => '<div class="form-group">' .
							'<div class="col-md-4">' . 
							'<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" ' . $aria_req . ' class="form-control"  placeholder="' .  __('Name', 'mosportfoliotemplate') . '" />' . 
							'</div>',
				'email'  => '<div class="col-md-4">' . 
							'<input id="email" name="email" ' . ($html5 ? 'type="email"' : 'type="text"') . ' value="' . esc_attr($commenter['comment_author_email']) . '" ' . $aria_req . ' class="form-control"   placeholder="' .  __('Email', 'mosportfoliotemplate') . '" />' . 
							'</div>',
				'url'    => '<div class="col-md-4">' . 
							'<input id="url" name="url" ' . ($html5 ? 'type="url"' : 'type="text"') . ' value="' . esc_attr($commenter['comment_author_url']) . '" class="form-control"   placeholder="' .  __('Website', 'mosportfoliotemplate') . '" />' . 
							'</div>' . 
							'</div>',
			),
			'comment_field' => '<div class="form-group">' .  
							'<div class="col-md-12">' . 
							'<textarea id="comment" name="comment" rows="8" aria-required="true" class="form-control"  placeholder="' .  __('Comment', 'mosportfoliotemplate') . '"></textarea>' . 
							'</div>' . 
							'</div>',
			'comment_notes_after' => '<p class="help-block">' . 
							//sprintf(__('You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', 'mosportfoliotemplate'), $comment_allowed_tags) . 
